  ...
  <div class="container mt-5">
    <div class="row">
      ...
      <div class="col-md-6derecha">
        <h2 class="text-center">Compra de Criptomonedas</h2>
        <form @submit.prevent="handlePurchase" class="mt-4">
          <div class="mb-3">
...
//import { saveTransaction } from "../Servicios/RestDBservice";
...
export default {
  ...
  methods: {
    async fetchCryptoPrice() {
      ...
      if (data) {
        // Usa el precio promedio de compra como referencia
        const prices = Object.values(data);
        const averagePrice = prices.reduce((sum, exchange) => sum + exchange.bid, 0) / prices.length;
    ...
    handlePurchase() {
      if (this.amount > 0) {
        ...
        const transaction = {
          ...
          action: "compra", // Tipo de acción
          crypto: this.selectedCrypto, // Criptomoneda
          amount: this.amount, // Cantidad de criptomoneda comprada
          money: this.totalInPesos, // Total en pesos
          datetime: new Date().toISOString(), // Fecha y hora de la compra
        };
        ...
        try {
          ...
          this.confirmationMessage = `Has comprado ${this.amount} ${this.selectedCrypto} exitosamente por $${this.totalInPesos}.`;

          // Reiniciar el formulario
          this.amount = 0;
        } catch (error) {
          // Mostrar mensaje de error
          this.confirmationMessage = "Ocurrió un error al guardar la transacción.";
